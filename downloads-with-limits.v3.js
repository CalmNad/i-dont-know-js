...
- при добавлении URL в фильтр он либо пропускает его дальше по цепочке, либо сохраняет во внутреннем хранилище (если условия пропуска не выполнены, например - превышено количество обрабатываемых url для домена);
...
<один проход по всем url, где мы "засыпаем" urls в цепочку фильров> => [фильтр на максимальное количество скачиваний по hostname] -> [фильтр на максимально количество одновременных скачиваний] -> [метод скачивания] -> ... (ожидание скачивания) ... -> запрос на следующий url к первому фильтру.
*/
const urls = [
